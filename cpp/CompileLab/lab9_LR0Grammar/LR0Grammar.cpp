...
void LR0Grammar::expand(Node *np, queue<Node*> &q) {
	...
	for (char ch: outs) {
		...
		if (np2 == _dfa.nps.end()) {
			...
			np->nexts.emplace(ch, newNp->status);
        	...
        	np->nexts.emplace(ch, (*np2)->status);
		}
	}
}
...
void LR0Grammar::printAnalysisTable() {
    ...
    for (Node *np: auxVec) {
        cout << "\n状态：" << np->status  << endl;
        cout << "后继："; 
        for (auto edge: np->nexts) cout << "[" << edge.first << ", " << edge.second << "] ";
        cout << endl;
        for (const Project &project: np->projects) {
	...
	cout << "\n分析表：\n\t\t\tACTION\t\t\t\t\tGOTO" << endl; 
	cout << "状态\t";
	for (char ch: V) cout << ch << "\t"; cout << "\t";
	for (char ch: _g.VN) if (ch != _g.S) cout << ch << "\t";
	cout << endl;
