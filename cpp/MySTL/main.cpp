...
*/
//error: no matching function for call to ‘travPost_R(BinNode<int>*, <unresolved overloaded function type>)’|
/*
...
#define DEFAULT_CAPACITY 4   //默认初始化空间大小
typedef int Rank;       //秩即为整形

template <typename T>     //栈结构
class Stack {
private:
        T *_elem;
        Rank _size;
...
public:
        Stack(): _capacity(DEFAULT_CAPACITY), _size(0) {   //构造
                _elem = new T[_capacity];
        }
        ~Stack() { delete[] _elem; }     //析构
        int size() { return _size; }         //数据量
        int capacity() { return _capacity; }   //ADT容量
        bool empty() { return !_size; }       //判空
        T top() { return _elem[_size - 1]; }   //栈顶
...
private:
        ...
        Rank rear;               //尾秩
protected:
        Rank rNext(Rank r) { return (r + 1) % _capacity; }  //秩在循环队列中下一位置
        void expand() {                             //扩容
...
public:
        CirQueue(): _capacity(DEFAULT_CAPACITY) {        //构造
                _elem = new T[_capacity];
                head = 0;
                rear = 0;
        }
        int size() { return (rear - head + _capacity) % _capacity; } //大小
        int capacity() { return _capacity; }                   //容量
        bool empty() { return head == rear; }  //判空
        ...
        T front() { return _elem[rNext(head)]; }   //首节点
        void show() {                                         //debug用
...
int main() {
        ...
        CirQueue<char> q;
        while (true) {
        cout << "请输入您的需要判断回文的字符串（\"#\"号结束）：";
        while (cin >> ch && ch != '#') {           //入栈和入队
                s.push(ch);
                q.enqueue(ch);
        }
        bool isPalindrome = true;
        while (!s.empty() && !q.empty()) {         //一一判断
                if (s.pop() != q.dequeue()) {
                        isPalindrome = false;
                        break;
                }
        }
        cout << (isPalindrome ? "该字符串是回文" : "该字符串不是回文") << endl;
    }
