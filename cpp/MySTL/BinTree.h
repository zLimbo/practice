...
void showTree(BinNodePosi(T) x) {
        ...
        while (!Q.empty()) {
                ...
                if (++cnt % 7 == 0) cout << "\n        ";
                cout << "(";
                if (x->lc) printf("%4d", x->lc->data);
                else printf("    ");
                cout << ",";
                printf("%4d", x->data);
                cout << ",";
                if (x->rc) printf("%4d", x->rc->data);
                else printf("    ");
                cout << ")  ";
                if (HasChild(*x)) {
                        if (HasLChild(*x) && x->lc->data > x->data) cout << "\nerror\n" << endl;
                        if (HasRChild(*x) && x->rc->data < x->data) cout << "\nerror\n" << endl;
                }
                if (HasLChild(*x)) Q.enqueue(x->lc);
                if (HasRChild(*x)) Q.enqueue(x->rc);
        }
...
void buildTree(const Vector<T> &TV, BinTree<T> &TTree) {
        ...
        while (cnt != TV.size()) {
                ...
                if (!HasLChild(*tmpBp)) {
                        TQ.enqueue( TTree.insertAsLC(tmpBp, TV[cnt++]) ); cout << tmpBp->lc->data << endl; }
                else if (!HasRChild(*tmpBp)) {
                       TQ.enqueue( TTree.insertAsRC(tmpBp, TV[cnt++]) ); cout << tmpBp->rc->data << endl; }
                else TQ.dequeue();
        }
