        ...
        for (int x = 0; x < LABY_MAX; ++x) {
                for (int y = 0; y < LABY_MAX; ++y) {
                        if (laby[x][y].status == WALL)
                                ...
                                if (laby[x][y].incoming == UNKNOWN || laby[x][y].outgoing == UNKNOWN)
                                        cout << "oo";
...
inline Cell* advance(Cell *cell) {
        Cell *next;
        switch(cell->outgoing) {
                case EAST : next = cell + 1;
                        next->incoming = WEST; break;
                case SOUTH : next = cell + LABY_MAX;
                        next->incoming = NORTH; break;
                case WEST : next = cell - 1;
                        next->incoming = WEST; break;
                case NORTH : next = cell - LABY_MAX;
