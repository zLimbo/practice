...
public:
	...
	int partFirstFit(int reqSize);
	int partNextFit(int reqSize);
 ...
 *
 */
void PartitionTable::createNewPartitionNode(PartitionNode *curr, int reqSize) {
	// 创建新分区节点
	int newStartAddr = curr->_startAddr + reqSize;
	int newSize = curr->_size - reqSize;
	PartitionNode *newPartition = new PartitionNode(
			assignId(), newSize, newStartAddr, FREE);
	// 按从小到大顺序选择位置插入新分区节点

	PartitionNode *temp =  _head->_next;
	while (temp != _tail && temp->_size <= newPartition->_size) {
			temp = temp->_next;
	}
	newPartition->_front = temp->_front;
	newPartition->_next = temp;
	newPartition->_front->_next = newPartition;
	newPartition->_next->_front = newPartition;
...
int PartitionTable::partFirstFit(int reqSize) {
	...
	if (_minSize <  curr->_size - reqSize ) {
		// 当前分区分割
		// 建立新分区节点插入分区表中
		createNewPartitionNode(curr, reqSize);
	} else {
		--_freeNum;
	}
	// 当前分区表状态为忙碌
...
int PartitionTable::partNextFit(int reqSize) {
	...
	if (_minSize <  _nfCurr->_size - reqSize ) {
		// 当前分区分割
		// 建立新分区节点插入分区表中
		createNewPartitionNode(_nfCurr, reqSize);
	} else {
		--_freeNum;
	}
	// 当前分区表状态为忙碌
 ...
 *
 */
 int  PartitionTable::partBestFit(int reqSize) {

 	return 0;
 }

 int PartitionTable::partWorstFit(int reqSize) {
